...
#define W2S(x,y) sdk->WorldToScreen(x, y)
...
public:
	...
	bool WorldToScreen(Vec3 coords_3d, Vec2& coords_2d);
	Vec2 WorldToScreen(Vec3 coords_3d);
	Vec3 TransformVec(Vec3 src, Vec3 ang, float d);

	Rect ConstructHeadBox2D(Vec2 top, Vec2 bottom);
	HyperRect ConstructHeadBox3D(Ent* ent);

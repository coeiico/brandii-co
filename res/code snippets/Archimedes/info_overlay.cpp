...
void Overlay::ShowEntsInfo() {
	...
	for (int i = 0; i < 32; i++) {
		...
		if (!sdk->CheckValidEnt(ent)) continue;
...
void Overlay::show_bone_ids(Ent* ent) {
	...
	for (int i = 0; i < 128; i++) {
		...
		Vec2 screen;
		if (W2S(sdk->GetBonePos(ent, i), screen)) {
...
void Overlay::show_crosshair_position() {
	...
	punch_angle = *(Vec3*) (uintptr_t*) (sdk->GetLocalPlayer() + offsets::m_aimPunchAngle);
...
void Overlay::show_player_model_name(Ent* ent, int pos) {
	...
	if (sdk->WorldToScreen(ent->m_vecOrigin, ent_position)) {
...
void Overlay::show_entity_id_info(Ent* ent, int pos) {
	...
	Vec2 ent_position;
	int crosshair_id = *(int*) (sdk->GetLocalPlayer() + offsets::m_iCrosshairId);
...
void Overlay::show_headbox_2d(Ent* ent) {
	...
	Vec3 ent_head_3d = sdk->GetBonePos(ent, 8); ent_head_3d.z += 8;
	Vec2 bottom;

	if (sdk->WorldToScreen(ent_head_3d, bottom)) {

		Vec2 top;
		if (sdk->WorldToScreen(sdk->GetBonePos(ent, 7), top)) {

			Rect head_box_2d;
			head_box_2d = sdk->ConstructHeadBox2D(top, bottom);
...
void Overlay::show_headbox_3d(Ent* ent) {
	...
	D3DCOLOR color = sdk->colors.color_white;
	HyperRect head_box_3d = sdk->ConstructHeadBox3D(ent);
